 ...
 * they look like a function definition, so ordinary text (a dataset `label`, a
 * title `text`) is never touched. Configs are trusted to contain Javascript
 * already - a config passed as a string is evaluated wholesale (see
 * `renderChartJs`), and README's "Securing your self-hosted instance" applies
 * either way - so this adds no capability that the API did not have.
 */
...
function walk(container, path) {
  Object.keys(container).forEach((key) => {
    ...
    if (typeof value === 'string') {
      if (FUNCTION_OPTION_KEYS.has(key) && FUNCTION_SOURCE.test(value)) {
