...
// Evaluating the source is what produces the value, so a source that *calls*
// something - `function() { ... }()` - has already run by the time its result
// is inspected. That is the trust model this whole file operates under (see the
// note at the top); what must not happen is the result being quietly discarded,
// because leaving the string in the config restores the silent misrender this
...
function compileSource(source, path) {
  ...
  if (typeof compiled !== 'function') {
    throw new ChartInputError(
      `Option "${path}" looks like a Javascript function but evaluates to ${describeValue(compiled)}. Pass the function itself, not a call to it.`,
    );
  }
  return compiled;
}
  ...
  Object.keys(container).forEach((key) => {
    ...
    if (typeof value === 'string') {
      if (takesFunction(key, path) && FUNCTION_SOURCE.test(value)) {
        container[key] = compileSource(value, childPath);
      }
      return;
    }
 ...
 * function but does not parse, or does not evaluate to one - the caller meant
 * it as code, and rendering it as a label instead would be a silent wrong
 * answer.
 */
