/**
 * Refuses Javascript quoted into a chart config.
 *
 ...
 * a config sent as JSON has no way to hold one. Callers reach for the obvious
 * next thing and quote the source:
 *
 *   { "plugins": { "datalabels": { "formatter": "function(v) { return v.y; }" } } }
 *
 * This server executes nothing a request sends it, so such a string can only be
 * refused or drawn. Drawn, it misrenders without a word: Chart.js and its
 * plugins see a string where they expect a function, the datalabels plugin
 * falls back to stringifying the raw datum ("[object Object]" for object data)
 * and a quoted `display` is simply truthy, so labels appear where the caller
 * meant to hide them. So it is refused, with a 400 naming the option.
 *
...
// text of the annotation, while `content` elsewhere is neither. Refusing these
// anywhere would turn a legend entry reading "x => y" into a 400.
    ...
    if (typeof value === 'string') {
      if (takesFunction(key, path) && FUNCTION_SOURCE.test(value)) {
        throw new ChartInputError(
          `Option "${childPath}" is Javascript source, which this server does not execute. ` +
            'Use plain values instead: a data row\'s "label" for label text, "ticks.format" ' +
            '(Intl.NumberFormat options) for number formats, an array of colors for per-point colors.',
        );
      }
      return;
    }
...
/**
 * Throws ChartInputError (HTTP 400) for a string in a chart config that is
 * Javascript source where Chart.js takes a function. Configs that are not
 * objects have nothing to walk and are left alone.
 */
function rejectFunctionStrings(chart) {
