...
// Ceiling on the squares the search *starts* from, which the one above cannot
// govern: it limits refining, and the starting grid is laid out before any
// refining happens.
//
// That grid is squares of the shape's narrow side, so the number of them is the
// shape's aspect ratio - and a caller's own GeoJSON can be any shape at all. A
// hair 900px long and a ten-thousandth of a pixel tall asks for six million
// squares up front, which is five seconds of one request; two more zeros in its
// coordinates and it is minutes and gigabytes.
//
// Starting coarser is safe, and costs a round or two of refining at most: every
// cell's bound covers its own square whatever the square's size, so the answer
// is bounded by `precision` either way.
const MAX_SEED_CELLS = 1024;
...
// A vertex, if that is what this is: a pair of finite numbers, returned as it
// came so a clean ring costs no copying. Coordinates reach here from
// caller-supplied GeoJSON and from a projection, and either can produce a string,
// a null, a missing member or a NaN.
function vertexOf(point) {
...
/**
 * The rings worth searching: every vertex a finite pair, and every ring left with
 * enough of them to enclose anything.
 *
 * Unusable vertices are dropped rather than taken as a reason to reject the ring
 * they are in - one bad coordinate says nothing about the rest of an outline, and
 * a shape missing a vertex still tells a label where to go.
 *
 * Filtered here rather than guarded further in, because a single NaN in the
 * arithmetic does not stay local. It makes one cell's distance NaN, every
 * comparison against that NaN false, and so the pruning that ends the search
 * stops ending it: the search runs to its cell ceiling and then reports nothing,
 * which for a 2000-vertex ring is 400ms spent to throw away a usable shape.
 */
function ringsOf(value) {
  ...
  return value
    ...
    .map((ring) => ring.map(vertexOf).filter((vertex) => vertex !== null))
 ...
 * each, in any order and wound either way (see `signedDistance`). Anything
 * `ringsOf` cannot read as a vertex is not part of the shape searched.
 */
function poleOfInaccessibility(rings, precision = PRECISION) {
  ...
  // Finite by now - every vertex measured above is - so a shape is only ruled out
  // for having no width or no height at all: a point, a line, a ring whose
  // vertices are collinear.
  if (Math.min(width, height) <= 0) {
    return null;
  }
  // The narrow side, or as much more as it takes to keep the starting grid to
  // MAX_SEED_CELLS squares.
  const cellSize = Math.max(Math.min(width, height), Math.max(width, height) / MAX_SEED_CELLS);
