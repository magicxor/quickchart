...
function describeGeometry(value) {
  ...
  const [[west, south], [east, north]] = d3geo.geoBounds(geo);
...
function autoProjectionSpec(outline, mapName) {
  ...
  const measured = describeGeometry(geo);
  if (!measured || !measured.centroid) {
    return { type: 'equalEarth' };
  }

  const [west, south, east, north] = measured.bbox;
  ...
  return {
    type: 'conicEqualArea',
    rotate: [round(-measured.centroid[0]), 0],
