...
function clippedPath(projection, extent) {
  ...
  return d3geo.geoPath({ stream: (sink) => projection.stream(rect.stream(sink)) });
}
...
function visibleAnchorFor(projection, extent) {
  ...
  return (feature) => {
    ...
    const point = tryMeasure((value) => path.centroid(value), best.part);
    return point && point.every(isFiniteNumber) ? point : null;
  };
}
