...
// Resolves a projection scale's `fit` into a GeoJSON value to frame the map
// on: a `[west, south, east, north]` box, a built-in map (optionally narrowed
// to named features), or inline GeoJSON.
function resolveFitGeometry(spec) {
  ...
  if (spec.map !== undefined) {
    const mapName = String(spec.map);
    const features = Array.isArray(spec.features)
      ? spec.features.map((key) => (typeof key === 'string' ? matchFeature(mapName, key) : key))
      : resolveOutline(mapName);
    if (features.length === 0) {
      throw new ChartInputError(`Projection "fit" selected no features from map "${mapName}"`);
    }
    return { type: 'FeatureCollection', features };
  }
  const geometry = toGeoJson(spec.features || spec);
  if (!geometry) {
    throw new ChartInputError(
      'Projection "fit" must be a [west, south, east, north] box, a { map, features } reference, or GeoJSON',
    );
...
// measured every time.
function autoProjectionFor(outline, mapName) {
  return buildProjection(
    mapName ? getMapGeography(mapName).projection : autoProjectionSpec(outline, null),
  );
}
...
// chartjs-chart-geo expects. Returns the plugins the chart needs.
function applyProjectionOptions(chart, outline, mapName) {
  ...
  if (isProjectionSpec(scale.projection)) {
    ...
    scale.projection = fitGeometry
      ? buildProjection(autoProjectionSpec(fitGeometry, null))
      : autoProjectionFor(outline, mapName);
  ...
  return fitGeometry ? [createFitPlugin(fitGeometry)] : [];
}
...
// values pass through untouched. Returns the plugins the chart needs.
function resolveGeoReferences(chart) {
  ...
  if (resolvedTypes.size > 0) {
    ...
    if (scales.projection === undefined) {
      ...
      scales.projection !== null &&
      typeof scales.projection === 'object' &&
      scales.projection.projection === undefined
    ) {
...
async function renderChartJs(
  ...
  const geoPlugins = resolveGeoReferences(chart);
