...
// The only thing a Javascript config is handed that depends on the canvas
// dimensions: `getGradientFill` and `getGradientFillHelper` close over them.
// Any mention counts, not just a call - a config may alias a helper before
// calling it, and the cost of being wrong in this direction is one extra
// evaluation, against a gradient built for the wrong canvas in the other.
const GRADIENT_HELPER_MENTION = /\bgetGradientFill(Helper)?\b/;
...
// values pass through untouched. Returns the plugins the chart needs, the
// framing for canvas sizing (see `applyProjectionOptions`), and which map
// features were left without data (see `describeGeoCoverage`).
function resolveGeoReferences(chart) {
  ...
  datasets.forEach((dataset) => {
    ...
    if (type === 'choropleth' && Array.isArray(dataset.data)) {
      const matched = matchMap ? matchedByMap.get(matchMap) || new Set() : null;
      ...
      if (matched) {
        matchedByMap.set(matchMap, matched);
      }
    }
  ...
  if (!isGeoChart) {
    return { plugins: [], framing: null, geoCoverage: null };
  }
  ...
  return { plugins, framing, geoCoverage: describeGeoCoverage(matchedByMap, frameBox) };
}
...
function prepareChart(untrustedChart, width, height) {
  ...
  const { plugins, framing, geoCoverage } = resolveGeoReferences(chart);
...
async function renderChartJs(
  ...
  const provisional = {
    width: isGivenSize(width) ? width : Math.min(DEFAULT_LONG_SIDE, MAX_WIDTH),
    height: isGivenSize(height) ? height : Math.min(DEFAULT_LONG_SIDE, MAX_HEIGHT),
  };
  ...
  if (
    ...
    GRADIENT_HELPER_MENTION.test(untrustedChart) &&
    ...
    // Nothing else the config is handed depends on the canvas, so no other
    // config is evaluated twice - which matters for one with side effects.
