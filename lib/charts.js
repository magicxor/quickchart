...
async function renderChartJs(
  ...
  if (height > MAX_HEIGHT) {
    throw `Requested height exceeds maximum of ${MAX_HEIGHT}`;
  }
  ...
  const chartInstance = new Chart(renderCanvas.getContext('2d'), chart);
  try {
    if (format === 'svg') {
      return Buffer.from(uniqueSvg(renderCanvas.toBuffer().toString('utf8')));
    }
    // Some controllers (force-directed graphs, trees) schedule extra layout
    // passes through the requestAnimationFrame shim. Let those settle before
