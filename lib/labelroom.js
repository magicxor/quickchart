...
// clipped label. Ink that has no room left under this floor gets the room
// there is and runs past it - showing most of every label is a better
// failure than hiding whole ones - while ink that could never fit at all
 ...
 * engine's judgement: unclip when some ink reaches outside and every
 * annotation, taken by itself, fits the axis budgets - so flag labels come
 * out whole while a box stretched far off-scale keeps today's clipped look.
 * That judgement is per annotation, not per chart: when opposite sides
 * together outgrow an axis budget, the reserve is prorated between them and
 * the leftover ink runs past it, which shows most of every label instead of
 * hiding whole ones. The decision is made once, on the first layout, and
 * written to the config so every later pass draws the same way.
 */
function buildChartWithLabelRoom(chart, size, reserve, buildChart) {
  ...
  for (let pass = 0; ; pass += 1) {
    ...
    // Reserves are whole pixels. Rounding both sides of a budget-clamped pair
    // up would take the pair a pixel back over the budget - and that pixel
    // comes out of the plot's guaranteed share - so it is taken back from the
    // side rounding gave the most to.
    const target = zeroSides();
    SIDES.forEach((side) => {
      target[side] = Math.round(needed[side]);
    });
    [
      ['top', 'bottom', 'y'],
      ['left', 'right', 'x'],
    ].forEach(([a, b, axis]) => {
      if (target[a] + target[b] > budgets[axis]) {
        const trimmed = target[a] - needed[a] >= target[b] - needed[b] ? a : b;
        target[trimmed] = Math.max(0, target[trimmed] - 1);
      }
    });

    let settled = true;
    SIDES.forEach((side) => {
      if (target[side] !== reserve[side]) {
        reserve[side] = target[side];
        settled = false;
      }
    });
