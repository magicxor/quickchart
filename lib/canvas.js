 ...
 * usually read at.
 */
const DEFAULT_LONG_SIDE = Number(process.env.CHART_DEFAULT_SIZE) || 1280;
...
function resolveCanvasSize({ width, height, ratio, chrome, maxWidth, maxHeight, longSide }) {
  ...
  // Neither given: the longest side is the target, the other one follows.
  return shape >= 1
    ? { width: clamp(target, maxWidth), height: clamp(heightFor(target), maxHeight) }
    : { width: clamp(widthFor(target), maxWidth), height: clamp(target, maxHeight) };
}
