 ...
 * usually read at.
 */
const FALLBACK_LONG_SIDE = 1280;

// The operator sets this, and it decides the size of every request that leaves
// a dimension open, so anything that is not a usable length falls back rather
// than propagating: `Number()` alone would let "-500" through as a length.
function parseLongSide(value) {
...
const DEFAULT_LONG_SIDE = parseLongSide(process.env.CHART_DEFAULT_SIZE);
