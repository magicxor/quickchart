 ...
 * nothing. Nor does the chart-level option have this problem - though note that
 * `applyQuickchartDefaults` rewrites a chart-level `false` to
 * `{ display: false }`, so switching the plugin off there does not switch off the
 * hook that trips over a dataset.
 */
