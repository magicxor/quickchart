 ...
 *
 * False only where the answer is settled by the config alone - the plugin turned
 * off, or `display: false` with nothing per dataset and no named label groups to
 * override it. That is the common case here, since every type but the axisless
 * ones defaults to no labels. `display` is scriptable otherwise, so anything
 * less definite counts as "could".
 */
function mayDrawDataLabels(chart) {
  ...
  if (datasets.some((dataset) => dataset && dataset.datalabels !== undefined)) {
    return true;
  }
  ...
  return options.display !== false || options.labels !== undefined;
}
