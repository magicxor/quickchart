...
describe('canvas sizing', () => {
  ...
  it('keeps gradients matched to the canvas it derived', async () => {
    // The gradient helpers close over the canvas extent, so a config that uses
    // them has to be re-evaluated once the extent is known.
    const config = `{
      type: 'bar',
      data: { labels: ['a', 'b'], datasets: [{ data: [5, 9],
        backgroundColor: getGradientFillHelper('vertical', ['#000000', '#ffffff']) }] }
    }`;
    const buf = await renderChartJs(undefined, undefined, 'white', 1, undefined, 'png', config);
    assert.deepStrictEqual(sizeOf(buf), { width: 1280, height: 720 });
  });
});
