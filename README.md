...
- A data row may also carry a `center` — `{ "longitude": …, "latitude": … }` — and that is the point its label is anchored to, instead of the one the server derives from the region's shape.  Reach for it when the shape itself has no good spot to write on: a crescent whose middle belongs to a neighbour (Croatia's centre of area is in Bosnia, Vietnam's in Laos), or a region that wraps another one entirely.  `{ "feature": "Croatia", "value": 39, "center": { "longitude": 16.4, "latitude": 45.3 } }` puts the label on Zagreb's side of the crescent rather than across the border.
